...
void draw_cashier(int item, int max_queue_length, Cashier cashier);
...
void draw_all_cashiers(int max_queue_length, int sum_cashiers, Cashier *cashiers) {
    for (int i = 0; i < sum_cashiers; ++i) {
        draw_cashier(i, max_queue_length, cashiers[i]);
    }
}
...
void draw_stop() {
    mvprintw(y_stop_text, 0, "Modeling system has been stop");
    refresh();
}
